...
export const createAddress =
  ...
  async (dispatch: any) => {
    dispatch({ type: ACTION_NAMES.CREATE_ADDRESS.CREATE_ADDRESS });
    const model = {
    ...
    if (response.data?.isSuccess) {
      data.onSuccess();
      dispatch({ type: ACTION_NAMES.CREATE_ADDRESS.CREATE_ADDRESS_SUCCESS });
    } else {
      data.onFailure();
    }
  };
...
export const setDefaultAddress =
  (id: string, onSuccess: () => void) => async (dispatch: any) => {
    dispatch({ type: ACTION_NAMES.SET_DEFAULT.SET_DEFAULT });

    const response = await api.post("/addresses/set-default?Id=" + id);
    if (response.data.isSuccess) {
      dispatch({ type: ACTION_NAMES.SET_DEFAULT.SET_DEFAULT_SUCCESS });
      onSuccess();
    } else {
      dispatch({ type: ACTION_NAMES.SET_DEFAULT.SET_DEFAULT_FAIL });
    }
  };
...
export const deleteAddress = (props: deleteProps) => async (dispatch: any) => {
  dispatch({ type: ACTION_NAMES.DELETE_ADDRESS.DELETE_ADDRESS });
  const response = await api.delete("/addresses?Id=" + props.id);
  if (response.data.isSuccess) {
    dispatch({ type: ACTION_NAMES.DELETE_ADDRESS.DELETE_ADDRESS_SUCCESS });
    props.onSuccess();
  } else {
    dispatch({ type: ACTION_NAMES.DELETE_ADDRESS.DELETE_ADDRESS_FAIL });
