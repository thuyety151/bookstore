        ...
        public class Handler : IRequestHandler<Command, Result<Unit>>
        {
            ...
            public async Task<Result<Unit>> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                if (wishlist == null)
                {
                    ...
                    {
                        Id = Guid.Parse(_httpContext.HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier)),
                        Items = new List<Item>()
                    };
                    await _context.WishLists.AddAsync(wishlist);
                }

                var item = wishlist.Items.FirstOrDefault(x =>
                ...
                else
                {
                    ...
                    wishlist.Items.ElementAt(index).Quantity = request.ItemParams.Quantity;
                    
                    if (DateTime.Now >= bookAttribute.SalePriceStartDate && DateTime.Now <= bookAttribute.SalePriceEndDate)
                    {
                        wishlist.Items.ElementAt(index).Price = bookAttribute.SalePrice;
                    }
                    else
                    {
                        wishlist.Items.ElementAt(index).Price = bookAttribute.Price;
                    }
                }
